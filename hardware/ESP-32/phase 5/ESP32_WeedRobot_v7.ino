/*
 ESP32_WeedRobot_v7.ino

 Fixes a real contract mismatch between this firmware and the Node
 backend (software/backend/services/esp32Service.js). The backend
 was already written expecting a specific set of endpoints -- this
 version makes the firmware match it. Nothing on the backend needs
 to change; only this file was wrong relative to what's already
 committed there.

   WHAT WAS MISMATCHED (v5/v6 vs what esp32Service.js actually calls):

     esp32Service.js call              v5/v6 firmware had         Fixed here
     ---------------------------------  --------------------------  ----------
     POST /move?dir=...                 GET  /move                  POST /move?dir=...
     POST /stop                         GET  /stop                  POST /stop
     POST /relay?state=on|off           GET  /cutter?state=on|off   POST /relay?state=on|off
     POST /mode?mode=auto|manual        GET  /mode?state=...        POST /mode?mode=auto|manual
     POST /speed?value=...              GET  /speed?value=...       POST /speed?value=...
     GET  /status, GET /ping            GET  /status, GET /ping     unchanged (already matched)

   The ESP32 WebServer library routes strictly by method + exact
   path, so every one of those mismatches would have returned a 404
   from the real board -- the backend could not have controlled the
   robot at all until this was fixed, cutter included.

 =====================================================================
 Everything below documents what earlier versions already did.
 =====================================================================

 v6 -- added a firmware-level cutter safety interlock (kept in this
 version, now on POST /relay instead of GET /cutter):
   Turning the relay ON is refused (HTTP 409) while avoidState !=
   AV_IDLE, i.e. while the robot is actively reversing or turning to
   avoid an obstacle. This is a hardware-level backstop:
   detect_stream.py already checks robot status over the network
   before requesting a cut, but that check runs on a separate laptop
   and can fail, crash, or be bypassed by any other caller hitting
   this endpoint directly. Because this check lives on the same
   board that switches the relay, it can't be skipped by anything
   upstream. Turning the relay OFF is always allowed unconditionally.

 v5 -- fixes and hardening on top of v4:
   ...
   2. ADDED: shared-key auth on every command endpoint. Callers send
      the key either as header "X-API-Key: <key>" or query param
      "?key=<key>". /status and /ping stay open (read-only).
      >>> CHANGE API_KEY BELOW BEFORE FIELD USE. <<<
   3. ADDED: CORS headers + OPTIONS preflight handler on every route,
      so a browser-based dashboard can call this ESP32 directly even
      across origins.
   4. ADDED: separate, shorter watchdog for the cutter/relay than for
      the drive motors (CUTTER_WATCHDOG_TIMEOUT_MS = 800ms vs
      WATCHDOG_TIMEOUT_MS = 2000ms for drive), so a stalled vision
      loop can't leave the blade spinning blind as long as it could
      leave the robot merely rolling forward.

 Note /mode still does NOT gate which source (dashboard vs vision
 script) is allowed to send commands -- that's a separate design
 ...
 publishing this repo. Must exactly match ESP32_API_KEY in
...
// >>> CHANGE THIS before field use. Must exactly match ESP32_API_KEY
// in software/backend/.env. Share it only with the dashboard backend
// and the YOLOv8 laptop script. <<<
...
// Relay/cutter: shorter window than drive, since a stalled blade over
// a weed for an extra second or two is a worse failure mode than the
...
unsigned long lastCutterCommandMs = 0; // updated on /relay (on) only
bool watchdogTripped = false;        // drive watchdog
bool cutterWatchdogTripped = false;  // relay/cutter watchdog
...
// Grouped by LEFT side (M1 front-left + M3 rear-left) vs RIGHT side
// (M2 front-right + M4 rear-right). Left side reverses, right side
...
// Relay/cutter watchdog: separate, shorter window. If the relay is on
// and hasn't received a refreshing POST /relay?state=on call within
...
void updateCutterWatchdog(){
  ...
  if (now - lastCutterCommandMs >= CUTTER_WATCHDOG_TIMEOUT_MS){
    if (!cutterWatchdogTripped){
      Serial.println("Cutter watchdog: no refresh received, forcing relay off.");
...
void setup(){
  ...
  // POST /mode?mode=manual|auto  -- bookkeeping only, doesn't gate commands
  // NOTE: param is "mode" (matches esp32Service.js), not "state".
  server.on("/mode", HTTP_POST, [](){
    ...
    if (server.hasArg("mode")){
      ...
      if (m == "manual" || m == "auto"){
        ...
        server.send(400, "text/plain", "mode must be 'manual' or 'auto'");
      }
    } else {
      server.send(400, "text/plain", "missing 'mode' arg (manual|auto)");
    }
  });
  ...
  // ---------------------------------------------------------------
  // POST /relay?state=on|off   (was GET /cutter?state=on|off)
  //
  // Renamed to /relay and switched to POST to match
  // software/backend/services/esp32Service.js's relay() function,
  // which is what the dashboard's cutter button and the future
  // POST /api/detection/weed handler both call through.
  //
  // SAFETY INTERLOCK (unchanged from v6): turning the relay ON is
  // refused (HTTP 409) while avoidState != AV_IDLE, i.e. while the
  // robot is actively reversing or turning to avoid an obstacle.
  // This is a hardware-level backstop: detect_stream.py already
  // checks robot status over the network before requesting a cut,
  // but that check runs on a separate laptop and can fail, crash, or
  // be bypassed by any other caller hitting this endpoint directly.
  // Because this check lives on the same board that switches the
  // relay, it can't be skipped by anything upstream.
  //
  // Turning the relay OFF is always allowed unconditionally -- a
  // stop/off request should never be blocked.
  // ---------------------------------------------------------------
  ...
  server.on("/relay", HTTP_OPTIONS, [](){ setCorsHeaders(); server.send(204); });
