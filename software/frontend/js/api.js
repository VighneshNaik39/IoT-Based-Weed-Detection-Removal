...
// ==========================================
// PHASE 5 - DETECTION API HELPERS
// Member 2 frontend helpers
// ==========================================
...
async function getDetectionStatus() {
  ...
  if (!response.ok) {
    throw new Error(`Detection status HTTP ${response.status}`);
  }
...
function getDetectionStreamUrl() {
  return "/api/detection/stream";
}

async function getDetectionStreamHealth() {
  try {
    const response = await fetch(getDetectionStreamUrl(), {
      method: "GET",
      cache: "no-store"
    });
...
// ==========================================
// PHASE 5 - CAMERA STREAM HEALTH
// ==========================================

async function getDetectionStreamHealth() {
    try {
        const response = await fetch(
            getDetectionStreamUrl(),
            {
                method: "GET",
                cache: "no-store"
            }
